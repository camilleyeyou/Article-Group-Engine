          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            <div className="bg-[#161b22] rounded-lg border border-[#30363d] overflow-hidden shadow-2xl">
              ...
              <div className="p-6 md:p-8 space-y-4">
                ...
                <div className="pl-6">
                  <motion.p className="text-2xl md:text-4xl font-bold text-white leading-tight" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                    {query}
                  </motion.p>
                </div>

                <motion.div className="flex items-center gap-2 text-emerald-400" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
                  <span>✓</span>
            ...
            <div className="grid grid-cols-3 gap-4 mt-8">
              {[
                ...
                <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 + i * 0.1 }} className="bg-[#161b22] border border-[#30363d] rounded-lg p-4">
          ...
          <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-center gap-3 mb-12">
              ...
              <h2 className="text-2xl font-bold">
                <span className="text-emerald-400">const</span> <span className="text-white">caseStudies</span> <span className="text-gray-500">=</span> <span className="text-cyan-400">[</span>
              </h2>
            </motion.div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {caseStudies.map((asset, i) => (
                <motion.div key={asset.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  <a href={`/asset/${asset.id}`} className="group block bg-[#0d1117] border border-[#30363d] rounded-lg p-6 hover:border-emerald-500/50 transition-all">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <Cpu className="w-5 h-5 text-emerald-400" />
                        {asset.client && <span className="text-xs px-2 py-1 bg-emerald-500/10 text-emerald-400 rounded">{asset.client}</span>}
                      </div>
                      <ChevronRight className="w-5 h-5 text-gray-600 group-hover:text-emerald-400 group-hover:translate-x-1 transition-all" />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-3 font-sans group-hover:text-emerald-100">{asset.title}</h3>
                    <p className="text-sm text-gray-400 line-clamp-3 font-sans">{asset.description || asset.content?.substring(0, 150) + '...'}</p>
                    <div className="mt-4 pt-4 border-t border-[#30363d]">
                      <code className="text-xs text-gray-500">id: &quot;{asset.id.substring(0, 8)}...&quot;</code>
                    </div>
                  </a>
                </motion.div>
              ))}
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-3">
              {articles.slice(0, 6).map((asset, i) => (
                <motion.div key={asset.id} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
                  <a href={`/asset/${asset.id}`} className="group flex items-center gap-4 p-4 rounded-lg hover:bg-[#161b22] transition-colors">
                    <Code2 className="w-5 h-5 text-gray-600 group-hover:text-emerald-400" />
                    <span className="text-gray-300 group-hover:text-white font-sans flex-1">{asset.title}</span>
      ...
      <section className="py-20 px-6 border-t border-[#30363d]">
        <motion.div className="max-w-3xl mx-auto text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div className="inline-block bg-[#161b22] border border-[#30363d] rounded-lg p-8 md:p-12">
            <code className="text-gray-500 text-sm block mb-4">{`// Ready to build something?`}</code>
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 font-sans">Let&apos;s ship it together</h2>
            <p className="text-gray-400 mb-8 font-sans">We speak developer. Let&apos;s create documentation, content, and experiences that resonate.</p>
            <a href="#" className="inline-flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-400 text-black font-semibold rounded-lg transition-colors">
