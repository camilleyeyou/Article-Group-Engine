 ...
 * 
 * This script:
 * 1. Reads PDFs from a local folder
 * 2. Uploads them to Supabase Storage
 * 3. Matches them to existing assets by title/filename
 * 4. Updates the asset's pdf_url
 * 
 * Usage:
 *   npx ts-node scripts/upload-pdfs.ts ./content/pdfs/
 */
...
async function findMatchingAsset(filename: string): Promise<{ id: string; title: string } | null> {
...
async function updateAssetWithPdfUrl(assetId: string, pdfUrl: string): Promise<void> {
  ...
  // Simpler update without metadata modification
  if (error) {
    ...
    if (simpleError) {
      throw new Error(`Failed to update asset: ${simpleError.message}`);
    }
  }
}
...
async function uploadAndLinkPDFs(directory: string): Promise<void> {
  ...
  // Process each PDF
  const results: { file: string; status: string; asset?: string; url?: string }[] = [];
...
USAGE:
  npx ts-node scripts/upload-pdfs.ts <pdf-directory>
